...
Hazelcast oltre a essere usato come database distribuito può essere usato per effettuare il distributed computing. Il distributed computing permette di distribuire il carico di lavoro tra i membri del cluster, migliorando le prestazioni e la scalabilità delle applicazioni.
...
Gli Executor Services offrono un framework per l'esecuzione di task in modo distribuito all'interno del cluster, estendendo il modello di concorrenza di Java con funzionalità distribuite.

=== Concetti fondamentali

È implementata l'interfaccia `java.util.concurrent.ExecutorService` con implementazioni distribuite che permettono di:
...
Per utilizzare un Executor Service, si può procedere come segue:
...
Gli Executor Services possono essere configurati nel file di configurazione:
...
Gli User Code Namespaces forniscono un meccanismo per organizzare e isolare il codice client che viene eseguito sui nodi del cluster, disponibile nella versione Enterprise.

=== Concetto e Scopo

Gli User Code Namespaces rappresentano un meccanismo fondamentale per gestire e isolare il codice eseguibile distribuito all'interno del cluster. Questo sistema risolve diverse sfide critiche nel computing distribuito:
...
2. *Hot Reloading:* È possibile ricaricare dinamicamente i namespace quando vengono rilevate modifiche nei jar o nelle classi associate.
...
L'utilizzo di Executor Services e User Code Namespaces offre numerosi vantaggi:
...
Il distributed computing permette di avvicinare l'elaborazione ai dati, riducendo la latenza e aumentando l'efficienza del sistema complessivo. Gli User Code Namespaces aggiungono un livello di gestione del codice che rende possibile la costruzione di sistemi modulari ed evolutivi che possono essere aggiornati e modificati senza interruzioni di servizio.
